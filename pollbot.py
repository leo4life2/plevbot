...
class PollBot:
    ...
    def _pollev_login(self) -> bool:
        ...
        """
        print("Logging into PollEv through pollev.com.")
    ...
    def _uw_login(self):
        ...
        print("Logging into PollEv through MyUW.")
    ...
    def answer_poll(self, poll_id) -> dict:
        ...
        r = self.session.post(
            ...
            headers={'x-csrf-token': self._get_csrf_token()},
            data={'value': "42", 'isPending': True, 'source': "pollev_page"}
        )
        return r.json()
    ...
    def run(self):
        ...
        while datetime.datetime.now().hour < 8:
            print("not 8am yet. sleep for 1 min.")
            time.sleep(60)
        ...
        while self.alive():
            ...
            if poll_id is None:
                print(f'`{self.host}` has not opened any new polls. '
                            f'Waiting {self.closed_wait} seconds before checking again.')
                time.sleep(self.closed_wait)
            else:
                print(f"{self.host} has opened a new poll! "
